...
export default function HomeServicesSection() {
  ...
  useEffect(() => {
    async function fetchServices() {
      setLoading(true);
      try {
        const data = await getAllServices<{ data: StrapiService[] }>(locale);
        const mappedServices: Service[] = data.data.map((service) => ({
          title: service.title,
          description: service.description,
          price: service.price,
          details: service.details,
          image: service.image?.[0]?.url
            ? `${environment.strapi.apiEndpoint}${service.image[0].url}`
            : '',
        }));
